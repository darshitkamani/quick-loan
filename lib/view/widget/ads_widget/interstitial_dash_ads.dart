...
class InterstitialAdsForDash {
  ...
  static Future loadHomeScreenIds({required BuildContext context}) async {
    ...
    print(
        "myAdsIdClassForHomeScreen --> ${myAdsIdClassForHomeScreen.availableAdsList}  ${myAdsIdClassForHomeScreen.facebookInterstitialId} ");
  }

  static Future<void> loadFBInterstitialAd(
      {required BuildContext context,
      required String routeFrom,
      required String screenName,
      required String? googleID,
      required MyAdsIdClass myAdsIdClass,
      required String? fbID}) async {
    final interstitialAdsWidgetProvider =
    ...
    print(
        'interstitialAdsWidgetProvider.isInterstitialAdLoaded ---> ${interstitialAdsWidgetProvider.isInterstitialAdLoaded}');
    ...
    FacebookInterstitialAd.loadInterstitialAd(
        placementId: kDebugMode
            ? 'IMG_16_9_APP_INSTALL#$fbInterstitialAdId'
            : fbInterstitialAdId,
        listener: (result, value) {
          debugPrint(
              'DASHBOARD RESULT fbInterstitialAdId - - - -- - - -- - $result ------- $value');
          ...
          if (result == InterstitialAdResult.ERROR) {
            ...
            bool isFacebookAdsShow =
                StorageUtils.prefs.getBool(StorageKeyUtils.isShowFacebookAds) ??
                    false;
            bool isADXAdsShow =
                StorageUtils.prefs.getBool(StorageKeyUtils.isShowADXAds) ??
                    false;
            bool isAdmobAdsShow =
                StorageUtils.prefs.getBool(StorageKeyUtils.isShowAdmobAds) ??
                    false;
            bool isAdShow =
                StorageUtils.prefs.getBool(StorageKeyUtils.isAddShowInApp) ??
                    false;
            if (routeFrom == 'SplashScreen') {
              if (isAdShow) {
                if (isAdmobAdsShow || isADXAdsShow) {
                  print(
                      'App Open called from facebook inter failed ${myAdsIdClass.googleAppOpenId}');
          ...
          if (result == InterstitialAdResult.DISMISSED &&
            ...
            if (screenName == 'DashboardScreen') {
              if (myAdsIdClassForHomeScreen.availableAdsList
                  .contains('Interstitial')) {
                myAdsIdClass = myAdsIdClassForHomeScreen;
                fbInterstitialAdId =
                    myAdsIdClassForHomeScreen.facebookInterstitialId;
                loadFBInterstitialAdForHomeScreen(
                    context: context,
                    myAdsIdClass: myAdsIdClassForHomeScreen,
                    screenName: 'HomeScreen',
                    fbID: myAdsIdClassForHomeScreen.facebookInterstitialId,
                    googleID: myAdsIdClassForHomeScreen.googleInterstitialId);
              }
            }
          }
        });
  ...
  static Future<void> loadFBInterstitialAdForHomeScreen(
      {required BuildContext context,
      required String screenName,
      required String? googleID,
      required MyAdsIdClass myAdsIdClass,
    ...
    FacebookInterstitialAd.loadInterstitialAd(
        ...
        listener: (result, value) {
          debugPrint(
              'Home screen result RESULT fbInterstitialAdId - - - -- - - -- - $result ------- $value');
          if (result == InterstitialAdResult.LOADED) {
            if (screenName == 'HomeScreen') {
              isInterstitialAdLoading = false;
              final interstitialAdsWidgetProvider =
  ...
  static showFBInterstitialAd(
      {BuildContext? context,
      String? screenName,
      String? googleID,
      String? fbID,
      MyAdsIdClass? myAdsIdClass,
      bool needToReLoadAd = false}) async {
